...
var FBRouterConstructor = function () {
     ...
     * @returns {string}
     */
    FBRouter.prototype.getShareUrl = function (withPages) {
        return this.adapter.getShareUrl(withPages);
    };
...
function FBPublicationVersion(options) {
    ...
    BasicPage.prototype._setSizes = function () {
        ...
        if (typeof FBInit.TOC_BOOK_HEIGHT !== 'undefined') {
            ...
            this.newRect = FBInit.detector.device.desktop ? this._getFitScreenSizes(this.pageRect.width, this.pageRect.height) : this._getFitWidthSizes(this.pageRect.width, this.pageRect.height);
    ...
    LibraryManager.superLoad(loaders, function (result) {
            ...
            if (currentIndex >= 0) //иначе это TOC
                {
                    ...
                    pagination.current.url = adapter.baseUrl + (adapter.getCurrentPage() === adapter.getFirstPage() ? "" : pagination.current.urlHeader + '/');
        ...
        function makeLink(index) {
            ...
            link.url = adapter.baseUrl + (urlHeader === adapter.getFirstPage() ? '' : urlHeader + '/');
        ...
        if (it.pageModel) {
            ...
            if (links && links.length > 0) {
                for (var i = 0; i < links.length; i++) {
                    ...
                    if (!url && id) {
                        var urlHeader = table.getUrlHeaderById(id).toString();
                        url = adapter.baseUrl + (urlHeader === adapter.getFirstPage() ? '' : urlHeader + '/');
                    }
                    var linkModel = {
                        type: "link",
                        url: url,
        ...
        LibraryManager.loadScript(adapter.translatePath('static/basic/js/localizator.js'), function () {});
...
PageModel.prototype.getThumbnailUrl = function () {
    var id = this.id,
        ...
        format = this.getProperty('thFormat'),
        isStub = this.isStub();
...
PageModel.prototype.getSubstrateUrl = function (size) {
    ...
    var id = this.id,
        imgStr = _fbUtils2.default.getNum(id),
        substrateFormat = format(this.getProperty('substrateFormat')),
        isStub = this.isStub();
